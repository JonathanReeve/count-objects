...
if __name__ == "__main__":
    ...
    for thisFile in allFiles:
        ...
        outfile = f"results-pg3/{basename}-annotated.txt"
        outfileOld = f"results-pg2/{basename}-annotated.txt"
        if os.path.isfile(outfile):
            logging.info(f"Skipping {thisFile}, since we already have it.")
            continue
        elif os.path.isfile(outfileOld):
            logging.info(f"Copying old annotated file from {outfileOld}")
            shutil.copy(outfileOld, outfile)
            continue
        try:
